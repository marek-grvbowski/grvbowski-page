(() => {
  ...
  function setContactPrompt(){
    ...
    if (key){
      termPrompt.textContent = `${key} >`;
      termInput.placeholder = key === 'message' ? 'write message' : `enter ${key}`;
    } else {
      termPrompt.textContent = 'cmd >';
      termInput.placeholder = 'submit / show / edit <field>';
    }
    termInput.focus();
  }
  ...
  function openEditField(key){
    ...
    if (idx === -1){
      termLine("unknown field. use: email, subject, message", 'warn');
      return;
    }
  ...
  function processContactInput(raw){
    ...
    if (lowered.startsWith('edit ')){
      openEditField(lowered.replace(/^edit\s+/, '').trim());
      return;
    }
    if (lowered === 'submit'){
      const missing = firstMissingField();
      if (missing){
        termLine(`missing ${missing} — complete all fields first`, 'warn');
        openEditField(missing);
        return;
      }
      if (!validateEmail(contactState.values.email)){
        termLine('email format looks invalid — edit email', 'warn');
        openEditField('email');
        return;
      }
    ...
    const key = contactState.order[contactState.idx];
    if (!key){
      termLine("unknown command. use: submit | show | edit <field>", 'warn');
      setContactPrompt();
      return;
    }
  ...
  function initContactTerminal(){
    ...
    typeTermSequence([
      { text: 'booting contact_me...', cls: 'sys', pause: 85 },
